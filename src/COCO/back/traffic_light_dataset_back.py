...
from Projects.CustomEval import CustomEval
...
class TrafficLightSubLightDetDataset(CocoDataset):
    CLASSES = ['TL_sub']
    ...
    def _prepare_for_json(self):
        ...
        if self.dataset_mode in ["train", "debug", "negative"]:
            ...
            bbox_filters.extend([
                bbox_filter_size_10,  # 过滤任意变长小于10的框
                bbox_filter_toward_orientation,  # 过滤没有朝向字段的框与非正面侧面的框
            ])
    ...
    def parse_haomo_annotation(self, annotation_json_path, *bbox_filters):
        ...
        for bbox in bboxes:
            ...
            if 'sub_lights' not in bbox.keys():
                continue
            for idx, sub_light in enumerate(bbox['sub_lights']):
                if sub_light['integrated'] != 0:
                    continue
                if len(bbox['sub_lights']) == 1:
                    x = bbox['bbox'][0]
                    w = bbox['bbox'][2]
                    y = bbox['bbox'][1]
                    h = bbox['bbox'][3]
                else:
                    assert len(bbox['splits']) > 0, "bug."
                    if bbox['pose_orientation'] == 1:
                        # 竖向
                        x = bbox['bbox'][0]
                        w = bbox['bbox'][2]
                        if idx == 0:
                            y = bbox['bbox'][1]
                            h = bbox['splits'][idx][-1] - bbox['bbox'][1]
                        elif idx == len(bbox['splits']):
                            y = bbox['splits'][-1][-1]
                            h = bbox['bbox'][1] + bbox['bbox'][3] - \
                                bbox['splits'][-1][-1]
                        else:
                            y = bbox['splits'][idx - 1][-1]
                            h = bbox['splits'][idx][-1] - \
                                bbox['splits'][idx - 1][-1]
                    elif bbox['pose_orientation'] == 0:
                        # 横向
                        y = bbox['bbox'][1]
                        h = bbox['bbox'][3]
                        if idx == 0:
                            x = bbox['bbox'][0]
                            w = bbox['splits'][idx][0] - bbox['bbox'][0]
                        elif idx == len(bbox['splits']):
                            x = bbox['splits'][-1][0]
                            w = bbox['bbox'][0] + bbox['bbox'][2] - \
                                bbox['splits'][-1][0]
                        else:
                            x = bbox['splits'][idx - 1][0]
                            w = bbox['splits'][idx][0] - \
                                bbox['splits'][idx - 1][0]
                temp_bbox_dict = dict()
                temp_bbox_dict['category_id'] = category_name_to_id['TL_sub']
                # x1, y1, w, h(这里我们默认w和h是包含起始点的，标注的规则里面是不包含起始点的，所以需要+1)
                temp_bbox_dict['bbox'] = x, y, w, h
                temp_bbox_dict['area'] = w * h
