...
def generate_head_feature_map_size(input_shape,pad = False):
    if pad:
        head_feature_map_sizes = [(math.ceil(input_shape[0] / 4), math.ceil(input_shape[1] / 4)),
        (math.ceil(input_shape[0] / 8), math.ceil(input_shape[1] / 8)),
        (math.ceil(input_shape[0] / 16), math.ceil(input_shape[1] / 16)),
        (math.ceil(input_shape[0] / 32), math.ceil(input_shape[1] / 32))] #,
    else:
...
def predict_onnx(image,
    ...
    # #out = np.array(res)
    # scores = proposal_bbox[0][:,4]
    # lens = len(np.where(scores>classification_threshold)[0])
    # bbox = proposal_bbox[0][:-1][:lens,:]
    # #print('1111',bbox)
    # return bbox
    # bg_label_padding = numpy.zeros((bbox[:,4].shape[0], 1))
    # #print(bbox.shape)
    # scores = bbox[:,4][:,np.newaxis]
    # scores = numpy.concatenate([scores, bg_label_padding], axis=1)
    # nms_cfg = {'iou_threshold': 0,
    #            'class_agnostic': class_agnostic}
    # nms_bboxes, nms_labels = multiclass_nms(
    #     multi_bboxes=torch.from_numpy(bbox[:,:4].astype(numpy.float32)),
    #     multi_scores=torch.from_numpy(scores.astype(numpy.float32)),
    ...
    #print('Re'*50,predicted_regression.max(),predicted_regression.min())
    ...
    #print('PRE',predicted_regression)
    ...
    predicted_regression = predicted_regression * concat_regression_ranges_max[..., None] #N_box * 4
...
def predclas(sess, img):
    ...
    pred_color, pred_shape, pred_toward = sess.run( # len=6,8,4
...
if __name__ == "__main__":
    ...
    for index1 in range(len(image_list)):
        ...
        bboxes_onnx = predict_onnx(im,
                         ...
                         expanded_regression_ranges_max,
                         location_embedding,
                         classification_threshold=0.35,#0.35
                         nms_threshold=0.4,#0.4
                         class_agnostic=False)
        print(len(bboxes_onnx))
        if isinstance(bboxes_onnx, list):
            ...
            if save_json:
                ...
                print("generate onnx detection output...",save_json_path, image_list[index1][:-4] + '.json')
                # continue
        else:
            if type(bboxes_onnx)!=numpy.ndarray:
                bboxes_onnx = bboxes_onnx.detach().numpy()# [x,x,x,x]+[p]
            det_list = []
            # im 1080*1920*3
            ...
            if save_json:
                with open(os.path.join(save_json_path, image_list[index1][:-4] + '.json'), "w") as f:
                    json.dump(det_list, f, indent=4)
                print("generate onnx detection output...",os.path.join(save_json_path, image_list[index1][:-4]))
