...
def match_bbox(label_bboxs, infer_bboxs, thres = 4):
    count = 0
...
def iou_bbox(label_bboxs, infer_bboxs, thres=0.5):
    count = 0
    for bbox in infer_bboxs:
...
def run(lable_path=input_lable_path, infer_path=input_infer_path):
    ...
    for infer_file in infer_files:
        infer_bboxs = read_txt(infer_file)
        ...
        if calculate_method == "pixel" :
            match_count += match_bbox(label_bboxs, infer_bboxs, pixel_debug_thres)
        elif calculate_method == "iou":
            match_count += iou_bbox(label_bboxs, infer_bboxs, iou_debug_thres)
